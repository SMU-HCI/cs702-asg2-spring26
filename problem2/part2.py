"""Part 2 — STL + Differentiable Programming: optimise trajectories.

Use the STL specifications from Part 1 as differentiable loss functions
and optimise trajectory positions via JAX gradient descent.
...
def total_stl_robustness(
    ...
    """Combined STL robustness using the specifications from Part 1.

    Imports and calls the robustness functions you implemented in part1.py.
    ...
    """
    from part1 import total_robustness
    return total_robustness(pos, traj_in, hotspots)
...
def separation_loss(pos: Any, min_dist: float = 8.0) -> Any:
    ...
    Returns:
        Scalar penalty.
    """
    # TODO (Part 2 — B): Implement the separation penalty.
    #
...
def run(
    ...
    # --- Initial positions (baseline — no procedural bundling) ---
    init_pos = baseline(traj_in)   # (K, N, 2)
    ...
    if record_dir is not None:
        ...
        export_frames(pos_opt, record_dir, hotspots=hotspots)
    else:
        from viewer import play
        play(pos_opt, title="Part 2 — Optimised Animation", hotspots=hotspots)
